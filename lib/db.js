...
function dataUrlToBuffer(dataUrl) {
  ...
  // Non-image payloads (audio) trust the declared mime; images keep magic-byte
  // detection because old clients labelled everything image/jpeg.
  const semi = dataUrl.indexOf(";");
  const declared = dataUrl.slice(5, semi === -1 ? dataUrl.indexOf(",") : semi);
  const mime = declared && !declared.startsWith("image/")
    ? declared
...
async function getPending(c = client()) {
  // Enrichable = never enriched AND has something to read (card photo or voice note).
  const { data, error } = await c.from("leads")
    .select("*").is("enriched_at", null)
    .or("front_url.not.is.null,audio_url.not.is.null");
